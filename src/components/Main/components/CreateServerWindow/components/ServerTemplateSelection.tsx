...
export default function ServerTemplateSelection(props:
{ updateActive: () => void, onServerTemplateSubmit: (input: string) => void }) {
    ...
    const templateButtons = serverTemplates.map(templateName => <CreateServerButton
        key={templateName}
        name={templateName}
        handleClick={props.onServerTemplateSubmit}
    />)

    return (
        <div className="relative bg-white w-108 h-auto rounded-md pt-6 grid grid-rows-[auto_auto_375px_auto]"
            ...
            <div className="w-full h-full px-4 pb-4 box-border overflow-y-scroll">
                <CreateServerButton name="Create My Own" handleClick={() => {props.onServerTemplateSubmit("Create My Own" )}}/>
